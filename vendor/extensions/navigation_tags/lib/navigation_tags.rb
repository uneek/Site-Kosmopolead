module NavigationTags
  ...
  tag "nav" do |tag|
    ...
    if @include_root
      ...
      tree = %{<li#{" class=\"#{css_class.join(" ")}\"" unless css_class.empty?}#{" id=\"" +
        (root.slug == "/" ? 'home' : root.slug) + "\"" if @ids_for_lis}><a href="#{root.slug}?p=accueil"#{" id=\"link_" + (root.slug == "/" ? 'home' : root.slug) + "\"" if @ids_for_links}>#{escape_once(root.breadcrumb)}</a></li>\n}
    else
      tree = ""
    end
  ...
  tag "sub-nav" do |tag|
    ...
    if child_page.children.select{|p| p.published?}.size>0 && !child_page.parent.parent.eql?(root)
      url = (defined?(SiteLanguage)  && SiteLanguage.count > 0) ? "/#{Locale.language.code}#{child_page.url}" :  child_page.children.select{|p| p.published?}.first.url
    else
